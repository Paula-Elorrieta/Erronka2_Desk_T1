...
public class BileraV extends JFrame {
	...
	public BileraV() {
		...
		btnVolver.setBounds(30, 340, 180, 30);
		panel.add(btnVolver);
		...
		try (Socket socket = new Socket(GlobalData.ZERBITZARIA_IP, Zerbitzaria.PUERTO);
			...
			if ("OK".equals(erantzuna)) {
				...
				for (Reuniones reunion : reunionesList) {
					model.addRow(new Object[] { reunion.getFecha().toLocalDateTime().toLocalDate().toString(),
							reunion.getFecha().toLocalDateTime().toLocalTime().toString(), reunion.getEstado(),
							reunion.getIdCentro(), reunion.getEstado().equals("pendiente") ? "Ikusi xehetasunak" : ""

					});
				}
			} else {
				JOptionPane.showMessageDialog(this, "Errorea bilerak aukeratzean.");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
