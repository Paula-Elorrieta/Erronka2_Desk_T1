...
public class DetallesReunionV extends JFrame {
	...
	public DetallesReunionV(Reuniones reunion) {
		...
		btnOnartu = sortuBotoia("Onartu", new Color(162, 119, 255), e -> onartuBilera(reunion));
		buttonPanel.add(btnOnartu);

		btnEzeztatu = sortuBotoia("Dezestatu", Color.RED, e -> ezeztatuBilera(reunion));
		buttonPanel.add(btnEzeztatu);

		if (reunion.getEstado().equalsIgnoreCase("aceptada") || reunion.getEstado().equalsIgnoreCase("denegada")) {
			btnEzeztatu.setEnabled(false);
			btnOnartu.setEnabled(false);
			btnEzeztatu.setBackground(Color.GRAY);
			btnOnartu.setBackground(Color.GRAY);
		}
	...
	private Ikastetxeak lortuIkastetxeaIdz(String idCentro) {
		try (Socket socket = new Socket(GlobalData.ZERBITZARIA_IP, Zerbitzaria.PUERTO)) {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Errore bat egon da ikastetxeko datuak lortzean: " + e.getMessage(),
	...
	private void ezeztatuBilera(Reuniones reunion) {
		try (Socket socket = new Socket(GlobalData.ZERBITZARIA_IP, Zerbitzaria.PUERTO)) {
			...
			out.writeObject("BILERA_UPDATE");
			out.writeObject(reunion);
			JOptionPane.showMessageDialog(null, "Bilera bertan behera utzi da behar bezala",
					"Bilera Bertan Behera Utzi", JOptionPane.INFORMATION_MESSAGE);
			dispose();
			new BileraV().setVisible(true);
		} catch (IOException e) {
			e.printStackTrace();
